...
<body class="bg-white p-6 text-gray-900" onload="window.print()">
    <div class="no-print mb-4 flex gap-3">
        ...
        <a href="{{ route('bills.measurement.pdf', $bill) }}" class="rounded-md border border-gray-300 bg-white px-4 py-2 text-xs font-semibold uppercase tracking-widest text-gray-700 hover:bg-gray-50">Download PDF</a>
    </div>
    ...
    <table class="mb-4 w-full border border-gray-800 text-xs">
        <tr class="border-b border-gray-800">
            <td class="border-r border-gray-800 p-2">Bill No.: {{ str_pad((string) $bill->invoice_no, 3, '0', STR_PAD_LEFT) }}</td>
            <td class="border-r border-gray-800 p-2">Date: {{ $bill->bill_date?->format('d/m/Y') }}</td>
            <td class="border-r border-gray-800 p-2">Ref No.: {{ $bill->ref_no }}</td>
            <td class="p-2">Ref Date: {{ $bill->ref_date?->format('d/m/Y') }}</td>
        </tr>
        <tr>
            <td class="p-2" colspan="4">Name Of Work: {{ $bill->subject }}</td>
        </tr>
    </table>
