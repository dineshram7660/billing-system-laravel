<x-admin-layout>
    <x-slot name="header">
        <h2 class="text-lg font-semibold text-gray-800">Account — {{ $account->account_name }}</h2>
    </x-slot>
    ...
    <div class="space-y-4">
        ...
        <div class="flex items-center justify-between">
            <a href="{{ route('accounts.index') }}" class="text-sm text-gray-600 hover:text-gray-900">&larr; Back to accounts</a>
        ...
        @can('create', \App\Models\AccountDetail::class)
            <div class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
                ...
                <form method="POST" action="{{ route('accounts.details.store', $account) }}" class="grid grid-cols-1 gap-4 sm:grid-cols-5">
                    @csrf
                    <div>
                    ...
                    @forelse ($details as $detail)
                        <tr>
                            ...
                            <td class="px-4 py-3 text-right">
                                @can('delete', $detail)
                                    <form method="POST" action="{{ route('accounts.details.destroy', [$account, $detail]) }}"
